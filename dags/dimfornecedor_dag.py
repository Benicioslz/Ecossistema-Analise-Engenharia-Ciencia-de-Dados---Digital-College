...
def etl_dim_fornecedor():
    ...
    def load_mssql(rows):
        ...
        insert_sql = "INSERT INTO #FornecedorStaging (id_dim_fornecedor, fornecedor, cnpj_fornecedor) VALUES (%s, %s, %s)"
        data = [(row["id_fornecedor"], row["fornecedor"], row["cnpj_fornecedor"]) for row in rows]
        cursor.executemany(insert_sql, data)
        
        # Aplica o MERGE
        merge_sql = """
            MERGE DataWarehouse.dbo.DimFornecedor AS Target
            USING #FornecedorStaging AS Source
            ON (Target.id_dim_fornecedor = Source.id_dim_fornecedor)
            ...
            WHEN NOT MATCHED BY TARGET THEN
                INSERT (id_dim_fornecedor, fornecedor, cnpj_fornecedor)
                VALUES (Source.id_dim_fornecedor, Source.fornecedor, Source.cnpj_fornecedor);
        """
