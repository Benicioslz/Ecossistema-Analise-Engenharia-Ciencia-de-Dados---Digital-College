...
def etl_dim_cliente():
    ...
    def load_mssql(rows):
        ...
        insert_sql = "INSERT INTO #ClienteStaging (id_dim_cliente, cliente, tipo_cliente) VALUES (%s, %s, %s)"
        data = [(row["id_cliente"], row["cliente"], row["tipo_cliente"]) for row in rows]
        cursor.executemany(insert_sql, data)
        
        # Aplica o MERGE
        merge_sql = """
            MERGE DataWarehouse.dbo.DimCliente AS Target
            USING #ClienteStaging AS Source
            ON (Target.id_dim_cliente = Source.id_dim_cliente)
            WHEN MATCHED THEN
                UPDATE SET 
                    Target.cliente = Source.cliente,
                    Target.tipo_cliente = Source.tipo_cliente    
            WHEN NOT MATCHED BY TARGET THEN
                INSERT (id_dim_cliente, cliente, tipo_cliente)
                VALUES (Source.id_dim_cliente, Source.cliente, Source.tipo_cliente);
        """
