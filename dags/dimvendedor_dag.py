...
def etl_dim_vendedor():
    ...
    def load_mssql(rows):
        ...
        insert_sql = "INSERT INTO #VendedorStaging (id_dim_vendedor, nome_vendedor, cpf) VALUES (%s, %s, %s)"
        data = [(row["id_vendedor"], row["nome_vendedor"], row["cpf"]) for row in rows]
        cursor.executemany(insert_sql, data)
        
        # Aplica o MERGE
        merge_sql = """
            MERGE DataWarehouse.dbo.DimVendedor AS Target
            USING #VendedorStaging AS Source
            ON (Target.id_dim_vendedor = Source.id_dim_vendedor)
            WHEN MATCHED THEN
                UPDATE SET 
            ...
            WHEN NOT MATCHED BY TARGET THEN
                INSERT (id_dim_vendedor, nome_vendedor, cpf)
                VALUES (Source.id_dim_vendedor, Source.nome_vendedor, Source.cpf);
        """
